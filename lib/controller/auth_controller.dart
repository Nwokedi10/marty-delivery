...
import 'package:sixam_mart_delivery/data/model/response/all_withs.dart';
import 'package:sixam_mart_delivery/data/model/response/companies.dart';
import 'package:sixam_mart_delivery/data/model/response/newBank.dart';
...
import 'package:sixam_mart_delivery/data/model/response/withdrawal_model.dart';
...
import '../data/model/response/bankUpdateInfo.dart';
import '../data/model/response/withdrawal_list_model.dart';
...
class AuthController extends GetxController implements GetxService {
  ...
  AllWithdrawalBalModel? _allWithdrawalBalModel;
  BankUpdateInfoModel? _newBankModel;
  ...
  List<WithdrawalListModel>? _withdrawals;
  ...
  AllWithdrawalBalModel? get allWithdrawalBalModel => _allWithdrawalBalModel;
  BankUpdateInfoModel? get newBankModel => _newBankModel;
  WithModel? get withModel => _withModel;
  ...
  List<WithdrawalListModel>? get withdrawalList => _withdrawals;
  ...
  Future<ResponseModel> withdraw(String amount) async {
    _isLoading = true;
    update();
    Response response = await authRepo.withdraw(amount);
    ResponseModel responseModel;
    if (response.statusCode == 200) {
  ...
  Future<ResponseModel> updateBank(String accountName, String bank, String accountNumber) async {
    _isLoading = true;
    update();
    Response response = await authRepo.updateBank(accountName, bank, accountNumber);
    ResponseModel responseModel;
    if (response.statusCode == 200) {
  ...
  Future<ResponseModel> cancelWithdraw(String id) async {
    _isLoading = true;
    update();
    Response response = await authRepo.cancelWith(id);
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      responseModel = ResponseModel(true, 'successful');
      print('${id} reversed');
  ...
  Future<ResponseModel> cancelBankUpdate(String id) async {
    _isLoading = true;
    update();
    Response response = await authRepo.cancelBankUpdate(id);
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      responseModel = ResponseModel(true, 'successful');
      print('${id} cancelled');
  ...
  Future<void> getWithdrawalStatus() async {
    ...
    try {
      Response response = await authRepo.getWithdrawalStat();

      if (response.statusCode == 200) {
        // Successful response
        _withModel = WithModel.fromJson(response.body);
        print(_withModel);
      } else {
  ...
  Future<void> getWithdrawalList() async {
    Response response = await authRepo.getWithdrawalList();
    if (response.statusCode == 200) {
      var parsedResponse = response.body;
      if(parsedResponse != null){
        var accountInfo = parsedResponse[0][0];
        var fullname = accountInfo['account_name'];
        var bank = accountInfo['bank'];
        var accnum = accountInfo['account_number'];
        _withdrawals = [];

        for (var withdrawal in parsedResponse[1]) {
          var id = withdrawal['id'];
          var amount = withdrawal['amount'];
          var createdAt = DateTime.parse(withdrawal['created_at']);
          var updatedAt = DateTime.parse(withdrawal['updated_at']);
          var status = withdrawal['status'];

          var withdrawalModel = WithdrawalListModel(
            id: id,
            fullname: fullname,
            amount: amount,
            status: status,
          ...
          _withdrawals?.add(withdrawalModel);
          _withdrawals = _withdrawals?.reversed.toList();
  ...
  Future<void> getTotalWith() async {
    Response response = await authRepo.getAllWiths();
    if (response.statusCode == 200) {
      var responseBody = response.body;
      if (responseBody != null) {
        _allWithdrawalBalModel = AllWithdrawalBalModel.fromJson(response.body[0]);
      } else {
        _allWithdrawalBalModel = AllWithdrawalBalModel(total: '0', pending: '0');
      }
  ...
  Future<void> newBankInfo() async {
    Response response = await authRepo.newBankInfo();
    if (response.statusCode == 200) {
      var responseBody = response.body;
      if (responseBody != null) {
        var parsedResponse = Map<String, dynamic>.from(responseBody);
        _newBankModel = BankUpdateInfoModel.fromJson(parsedResponse);
      }
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  Future<void> getCompanyList() async {
    Response response = await authRepo.getCompanies();
    if (response.statusCode == 200) {
      _companyList = [];
      response.body.forEach((company) => _companyList!.add(CompanyModel.fromJson(company)));
    } else {
      ApiChecker.checkApi(response);
    }
    update();
